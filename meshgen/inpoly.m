...
%   Last updated   : 23/05/2007 with MATLAB 7.0
%
...
for k = 1:nc         % Loop through edges
   ...
   if y(1)>=y1
      start = 1;
   elseif y(n)<=y1
      start = n+1;        % Higher than all points, don't test any!
   else
      lower = 1;
      upper = n;
      for j = 1:n
   ...
   % Choose a tolerance for each edge based on the bbox size
   tol = 100*eps*(y2-y1+xmax-xmin);
